...
def start_session():
    result = supabase.from_('system.crawl_sessions').insert({
        'status': 'RUNNING'
    }).execute()
...
def save_document(data: dict, session_id: int):
    ...
    # 1. Upsert doc_urls
    supabase.from_('system.crawl_url').upsert({
        'url': url,
        'doc_id': doc_id,
        'title': title,
        'status': 'completed',
    ...
    # Check existing
    existing = supabase.from_('tvpl.document_finals').select('hash, update_date').eq('doc_id', doc_id).execute()
    ...
    if has_changed and old_hash:
        ...
        if new_total == 0:
            old_version = supabase.from_('tvpl.document_versions')\
    ...
    if has_changed:
        ...
        supabase.from_('tvpl.document_finals').upsert({
            'doc_id': doc_id,
            'title': title,
            'url': url,
            'hash': content_hash,
        ...
        supabase.from_('tvpl.document_versions').insert({
            'doc_id': doc_id,
        ...
        supabase.from_('tvpl.document_relations').delete().eq('source_doc_id', doc_id).execute()
        ...
        for relation_type, targets in relations.items():
            ...
            for target in targets:
                ...
                supabase.from_('tvpl.document_relations').insert({
        ...
        if tab8_links:
            # Delete old files
            supabase.from_('tvpl.document_files').delete().eq('doc_id', doc_id).execute()
            ...
            for link in tab8_links:
                ...
                supabase.from_('tvpl.document_files').insert({
                    'doc_id': doc_id,
                    'file_name': file_name,
                    'file_type': file_type,
                    'file_url': file_url,
        ...
        supabase.from_('tvpl.document_finals').update({
            'title': title,
            'last_crawled': datetime.now().isoformat()
