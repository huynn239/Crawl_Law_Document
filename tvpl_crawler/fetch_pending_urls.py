...
def fetch_pending_urls(limit=None, output_file=None):
    ...
    while True:
        # Get pending first
        query = supabase.from_('system.crawl_url').select('url, title, last_update_date, retry_count').eq('status', 'pending').order('priority', desc=True).range(offset, offset + page_size - 1)
