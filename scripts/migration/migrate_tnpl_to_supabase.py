...
def migrate_tnpl_terms():
    ...
    for i in range(0, len(rows), batch_size):
        ...
        try:
            result = supabase.table('terms').insert(data).execute()
...
def migrate_tnpl_sessions():
    ...
    try:
        result = supabase.table('crawl_sessions').insert(data).execute()
...
def verify_migration():
    ...
    result = supabase.table('crawl_sessions').select('session_id', count='exact').execute()
    sessions_count = result.count
    print(f"  ✓ tnpl_crawl_sessions: {sessions_count} records")
    
    # Sample data
    result = supabase.table('terms').select('*').limit(3).execute()
