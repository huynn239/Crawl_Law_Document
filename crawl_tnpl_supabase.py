...
def start_session():
    """Tạo session mới"""
    result = supabase.from_('tnpl.crawl_sessions').insert({
...
def save_terms_batch(terms, session_id):
    ...
    existing = supabase.from_('tnpl.terms').select('url, term_name, definition').in_('url', urls).execute()
    ...
    if new_terms:
        try:
            ...
            supabase.from_('tnpl.terms').insert(data).execute()
    ...
    for term in update_terms:
        try:
            supabase.from_('tnpl.terms').update({
...
def complete_session(session_id, total, new, updated, status='COMPLETED'):
    """Hoàn thành session"""
    supabase.from_('tnpl.crawl_sessions').update({
