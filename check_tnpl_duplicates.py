...
try:
    # Lấy tất cả URLs
    result = supabase.from_('tnpl.terms').select('url').execute()
    ...
    if duplicates:
        ...
        for url, count in list(duplicates.items())[:10]:
            ...
            # Lấy chi tiết
            result = supabase.from_('tnpl.terms').select('term_id, term_name, created_at').eq('url', url).execute()
...
try:
    result = supabase.from_('tnpl.terms').select('term_name').execute()
